...
const App: React.FC = () => {
  ...
  return <div className="min-h-screen desk-texture text-walnut flex flex-col">{renderView()}</div>;
};
    ...
    return (
        <div className="flex flex-col flex-1 h-screen overflow-hidden relative">
            <header className="px-6 py-4 flex items-center justify-between border-b border-walnut/10 bg-parchment-base shrink-0 z-40">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      <h1 className="text-[11px] font-black tracking-[0.25em] uppercase leading-none text-walnut">Cinematic Mirror</h1>
                      <p className="text-[7px] font-mono tracking-widest text-walnut/40 uppercase mt-0.5">Studio Archives 1974</p>
                    </div>
                </div>
            ...
            <nav className="fixed bottom-0 left-0 right-0 max-w-[430px] mx-auto bg-parchment-light/95 backdrop-blur-xl border-t border-walnut/10 px-6 pt-4 pb-8 flex justify-around items-center z-50 shadow-[0_-15px_50px_rgba(61,43,31,0.15)]">
                <NavItem icon="auto_stories" label="穿搭图鉴" active={view === View.STYLING} onClick={() => navigate(View.STYLING)} />
                <NavItem icon="ink_pen" label="导演咨询" active={view === View.DASHBOARD} onClick={() => navigate(View.DASHBOARD)} />
                <NavItem icon="explore" label="电影广场" active={view === View.PLAZA} onClick={() => navigate(View.PLAZA)} />
                <NavItem icon="person_book" label="我的档案" active={view === View.PROFILE} onClick={() => navigate(View.PROFILE)} />
            </nav>
        </div>
    );
}
