...
const Dashboard: React.FC<{ profile: PersonalityProfile | null }> = ({ profile: latestProfile }) => {
  ...
  const [currentSubtitle, setCurrentSubtitle] = useState(""); // 当前轮次字幕（每轮结束后重置）
...
使用中文回复。`;
      ...
      await geminiLive.connect({
        ...
        onTextResponse: (text) => {
          // 累积当前轮次字幕（每次 turnComplete 或 interrupted 后重置）
        ...
        onTurnComplete: () => {
          ...
          setCurrentSubtitle(prev => {
            if (prev.trim()) {
              setMessages(m => [...m, { role: 'model', text: prev }]);
            }
            return '';
          });
        },
  ...
  const startMediaCapture = async () => {
    try {
      ...
      // 复用 geminiLive 的 AudioContext，避免 iOS 同时存在两个 AudioContext 导致播放被挂起
      const ctx = geminiLive.getAudioContext();
      ...
      processor.onaudioprocess = (e) => {
        ...
        if (inputRate === targetRate) {
          ...
          // 降采样到 16000 Hz（iOS 实际采样率可能是 24000/44100/48000）
          const ratio = inputRate / targetRate;
          const outputLength = Math.floor(inputData.length / ratio);
          const pcm16 = new Int16Array(outputLength);
          for (let i = 0; i < outputLength; i++) {
            const srcIdx = Math.min(Math.floor(i * ratio), inputData.length - 1);
            pcm16[i] = Math.max(-32768, Math.min(32767, inputData[srcIdx] * 32768));
          }
          geminiLive.sendAudio(pcm16.buffer);
        }
      };

      // 通过静音 gain 连接 destination：让 onaudioprocess 正常触发，
      // 但不把麦克风音频输出到扬声器（避免 iOS 误判为反馈并触发回声抑制，
      // 从而影响 Gemini 音频播放）
  ...
  const cleanupLiveSession = () => {
    ...
    // AudioContext 由 geminiLive 统一管理，不在此关闭
        ...
        {currentSubtitle && (
          <div className="absolute left-4 right-4 bottom-44 z-10">
            <div className="bg-black/50 backdrop-blur-xl rounded-2xl px-5 py-4 border border-white/10 shadow-2xl">
              ...
              {parseModelResponse(currentSubtitle).map((part, idx) => (
                part.type === 'action' ? (
