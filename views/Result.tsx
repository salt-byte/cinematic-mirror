      ...
      <div className="flex-1 overflow-y-auto pb-60 bg-parchment-base animate-in fade-in duration-1000 no-scrollbar relative">
         ...
         <header className="pt-16 px-10 flex justify-between items-end border-b border-walnut/10 pb-8 relative">
            <div className="space-y-2">
               <div className="flex items-center gap-2">
                  ...
                  <p className="font-mono text-[8px] text-walnut/30 uppercase tracking-[0.4em]">{t('result.dossier')} #LY_{profile.id?.slice(0, 4).toUpperCase()}</p>
               </div>
            ...
            <div className="absolute top-12 right-6 -rotate-12">
               <Stamp text="VERIFIED" subText={t('result.verified').split(' / ')[0]} className="opacity-70 scale-90" />
            </div>
         </header>
         ...
         <section className="mt-12 px-10">
            <div className="bg-white/50 p-10 shadow-stack border border-walnut/5 relative overflow-hidden">
               <div className="flex items-center gap-2 mb-8">
                  ...
                  <h2 className="font-black text-[10px] uppercase tracking-[0.4em] text-walnut/30">{t('result.corePortrait')}</h2>
               </div>
         ...
         <section className="mt-20 px-10">
            <div className="flex items-center gap-4 mb-12">
               <h3 className="font-retro font-black text-sm text-walnut/40 tracking-[0.5em] uppercase">{t('result.directorNotes')}</h3>
            ...
            <div className="bg-ink p-12 relative overflow-hidden rounded-sm">
               ...
               <div className="relative z-10 space-y-16">
                  <header className="text-center">
                     <h3 className="font-retro font-black text-xs text-parchment-base/30 tracking-[0.6em] uppercase">{t('result.castingMatches')}</h3>
                  </header>
         ...
         <div className="fixed bottom-0 left-0 right-0 p-10 bg-gradient-to-t from-parchment-base via-parchment-base to-transparent z-50 max-w-[430px] mx-auto">
            ...
            >
               <span className="text-[8px] font-bold tracking-[0.5em] uppercase opacity-40 group-hover:opacity-60">{t('result.proceedHint')}</span>
               <span className="text-sm font-retro font-black tracking-[0.4em] uppercase">{t('result.proceedToStyling')}</span>
            </button>
         </div>
      </div>
   );
